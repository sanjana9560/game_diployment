...
function option(key){
  musicSound.pause();
    ...
    console.log(questionIndex,"sddsfddsf")
    console.log(questions[questionIndex]);
    console.log('Que',questions[questionIndex].optionList[key-1].Right_Ans);
    startTimer(false);
    musicSound.pause();
    if(1 == questions[questionIndex].optionList[key-1].Right_Ans){
    let element =  document.getElementById("quiz");
        scores+=1;
        // console.log(scores);
    ...
    questionIndex++;
    // let option  = document.getElementById("option-container");
    // option.innerHTML="";
    let optionContainer = document.getElementById("option-container");
    optionContainer.innerHTML = "";
}
...
function addQuiz(){
    ...
    for (let i = 0; i < questions[questionIndex].optionList.length; i++) {
    option.innerHTML+=`<button class="option" onclick="option(${i+1})"><span id="label1">${questions[questionIndex].optionList[i].Answer_Description}</span></button>`
    }
